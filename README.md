...
전에는 상대 서버를 배려해 업무 시간(KST 08~21시)만 10분, 그 외에는 60분으로
두었다. 그런데 실측해 보니 야간 슬롯에서 발견된 공고가 실제로 있었고
(9/3 08:01, 9/4 22:01), 그 건들은 게시 후 최대 1시간이 지나서야 메일이 나갔다.

한공회 게시판은 등록일자를 `2026.09.04` 처럼 **날짜로만** 주고 시각이 없다.
그래서 "게시 → 발견" 지연은 측정할 방법이 없고, **크롤 주기가 곧 그 상한**이다.
10분을 지키려면 주기 자체가 10분이어야 한다.

야간에 늘어나는 건 목록 1페이지 조회 하루 55회분이고 상세는 신규 건만 보므로,
한공회 서버에 주는 부담은 무시할 수 있는 수준이다.
...
읽지도 쓰지도 않는다. 10분마다 도는 크롤이 손으로 채운 자료를 덮어쓰면 안 된다.
