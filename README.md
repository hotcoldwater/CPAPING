...
| 구인(CPA) | `/home/jobOffrSrchGnrl/` | 경력·개업 위주이나 기장·세무대리 직원 공고가 절반 이상. 2026-09-09 부터 매분 수집, 회계사 공고(`audience=cpa`)만 대상 (Phase 9) |
...
공고 상세에는 **요약 + 공개된 채용 본문·이미지·지원 연락처 + 첨부 안내 + 한공회 원문 링크**를 제공한다.
외부 HTML은 실행하지 않고 허용된 문서 구조로 저장·출력한다. 구독자 알림 메일은 기존 요약과 링크를 유지한다.
본문 확인 시각은 목록 확인 시각과 구분한다. 기존 공고는 게시판별 회차당 최대 1건씩, 24시간이 지난 내용을 갱신한다.
...
| **6** | 커뮤니티 2 — 게시판(채용·이직 / 시험·수험 / 현직 / 자유), 추천, 카테고리 | **자유게시판 1차 2026-09-10** — 글·댓글(대댓글·추천·베스트 그대로)·글 추천·글 신고. 카테고리는 다음 |
| **7** | 자소서 — 공고 '지원 요건' 블록·`/guide`(선행) → 재료 4시트 → 자유양식 → 법인 맞춤 → 가져오기 | 계획 확정 |
| **8** | 채널·비용 — 카카오 알림톡(사업자 필요), 요약 옵션, SES 이전 | |
| **9** | 경력 공고 확장 — 한공회 구인(CPA) 게시판. 결정(2026-09-09): 경력은 빅4 포함 · 기장·세무대리 등 직원 공고 제외 · 일반기업 회계사 공고 포함 · 알림은 수습 정규/수습 파트/경력 각각 선택. 1단계 수집·분류(`audience`, 연차) → 2단계 알림 스위치·회원 화면 → 3단계 홈 탭·`/career/`·상세 | **1·2·3단계 배포 2026-09-10** — 알림 스위치 3개, 경력 메일 [경력]·연차, 홈 종류 필터(수습 / 수습 파트 / 경력)와 칩, 경력 상세·법인 페이지 공고, 약관·방침 문구. `/career/` 검색 노출 페이지는 다음 |
...
- `/posting/<ij_id>/` — 공고 상세. 목록에서 공고를 누르면 한공회로 바로 나가지 않고
  여기로 온다. 최신 DB 정보를 서버 렌더링하며, 요약 아래에 본문·이미지·지원 연락처·첨부 안내를 보여준다.
  한공회 전용 다운로드 절차가 필요한 첨부는 원문 페이지로 연결한다. DB 장애 때는 배포 시 만든 사본을 사용한다.
