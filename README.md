...
| **9** | 경력 공고 확장 — 한공회 구인(CPA) 게시판. 결정(2026-09-09): 경력은 빅4 포함 · 기장·세무대리 등 직원 공고 제외 · 일반기업 회계사 공고 포함 · 알림은 수습 정규/수습 파트/경력 각각 선택. 1단계 수집·분류(`audience`, 연차) → 2단계 알림 스위치·회원 화면 → 3단계 홈 탭·`/career/`·상세 | **1·2단계 배포 2026-09-10** — 알림 스위치 3개(온보딩·내 계정·토큰 설정 화면), 경력 메일 [경력]·연차, 수습 메일 하단에 경력 알림 안내 한 줄. 화면(3단계)은 아직 수습만 |
