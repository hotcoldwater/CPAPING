...
test('wizard validates the agreed order and only starts automation at final confirmation',async()=>{
 const s=await wizard();try{const {$,d}=s;const writes=()=>s.calls.filter(c=>c.url.endsWith('/resume-rule'));assert.deepEqual([...d.querySelectorAll('.resume-progress button')].map(b=>b.lastElementChild.textContent),['이력서','지원 조건','메일 작성','Gmail 연결','발송 방식','최종 확인']);assert.equal(d.querySelectorAll('[data-resume-step]:not([hidden])').length,1);s.next();assert.equal($('resume-form').dataset.step,'1');$('resume-file-id').value=uid;$('resume-docx-file-id').value=other;s.next();$('resume-full').checked=false;$('resume-part').checked=false;s.next();assert.equal($('resume-form').dataset.step,'2');$('resume-part').checked=true;s.next();s.next();assert.equal($('resume-form').dataset.step,'3');$('applicant-name').value='지원자';s.next();s.next();await $('resume-form').onsubmit({preventDefault(){}});assert.equal(writes().length,0);assert.equal($('resume-form').dataset.step,'6');await $('resume-form').onsubmit({preventDefault(){}});assert.equal(writes().length,0);$('resume-confirmed').checked=true;await $('resume-form').onsubmit({preventDefault(){}});assert.equal(writes().length,1);assert.equal(writes()[0].body.enabled,true);assert.deepEqual(writes()[0].body.filters,{employment:['Part Time']});}finally{s.close();}
});
test('missing automatic consent returns to its step and focuses the visible control',async()=>{
 const s=await wizard();try{const {$,d}=s;$('resume-file-id').value=uid;$('resume-docx-file-id').value=other;$('applicant-name').value='지원자';for(let i=0;i<4;i++)s.next();d.querySelector('[name=send-mode][value=auto]').click();s.next();$('resume-confirmed').checked=true;await $('resume-form').onsubmit({preventDefault(){}});assert.equal($('resume-form').dataset.step,'5');assert.equal(d.activeElement.id,'resume-consent');assert.equal(s.calls.filter(c=>c.url.endsWith('/resume-rule')).length,0);}finally{s.close();}
});
test('save API has no daily quota input requirement and ignores a legacy client cap',async t=>{
 ...
 for(const legacy of [{},{daily_limit:1}]){const data={resume_file_id:other,resume_docx_file_id:uid,applicant_name:'지원자',subject:'제목',body:'본문',filters:{employment:['Full Time']},mode:'review',enabled:false,file_confirmed:true,...legacy};
