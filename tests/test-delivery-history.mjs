...
test('history renders email as text, exposes no tracking image and shows uncertainty',async()=>{
 ...
 dom.window.fetch=async url=>({ok:true,json:async()=>String(url).includes('application-history')?{items:[{id:uid,company:'테스트 법인',recipient:'x@example.com',subject:'<img onerror=alert(1)>',body:'<script>bad()</script>',status:'sent',mode:'auto',first_open_at:'2026-09-13T00:00:00Z'}],has_more:false}:{mail:{email:'self@example.com'}}});
 dom.window.HTMLDialogElement.prototype.showModal=function(){this.open=true;};dom.window.HTMLDialogElement.prototype.close=function(){this.open=false;};
 dom.window.eval(readFileSync('web/applications.js','utf8'));await new Promise(r=>setTimeout(r,35));const d=dom.window.document;assert.equal(d.querySelectorAll('#deliveries img,#deliveries script').length,0);assert.match(d.getElementById('deliveries').textContent,/읽음 추정/);d.querySelector('#deliveries button').click();await new Promise(r=>setTimeout(r,20));assert.match(d.getElementById('application-detail').textContent,/<script>bad/);assert.equal(d.querySelectorAll('#application-detail script,#application-detail img').length,0);dom.window.close();
});
