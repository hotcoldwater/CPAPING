...
import {resumeUpload,mailTemplate} from '../functions/_resume.js';import {onRequest} from '../functions/api/career/[[path]].js';
...
test('resume UI safely renders file names and defaults to review with automation off',async()=>{
 const dom=new JSDOM(readFileSync('web/applications.html','utf8'),{url:'https://cpaping.com/applications/',runScripts:'outside-only'}),w=dom.window;
 ...
 w.eval(readFileSync('web/resume.js','utf8'));await new Promise(r=>setTimeout(r,40));assert.equal(w.document.querySelectorAll('#resume-files img').length,0);assert.equal(w.document.getElementById('resume-enabled').checked,false);assert.equal(w.document.getElementById('resume-mode').value,'review');assert.equal(w.document.getElementById('resume-consent').checked,false);dom.window.close();
});
