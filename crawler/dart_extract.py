...
def parse_revenue(text: str) -> dict:
    ...
    **바로 앞의 부문 이름으로 짝짓는다.**
    ...
    # 있는 값에서 계산하는 것이다.
    # 당기 금액과 비중만 읽고 전기·전전기는 보지 않는다.
    #
    # 처음에는 세 기간을 모두 요구했는데, PDF 에서 글자를 뽑을 때 줄 끝이
    # 잘리는 보고서가 많다. 공감 2024.03 의 회계감사 소계가
    # '소계 3,112,229,177 35.17 2,953,390,91' 에서 끊긴다. 당기 값은
    # 멀쩡한데 뒤가 잘렸다는 이유로 부문 전체를 0 으로 버리고 있었다.
    # 어차피 쓰지 않는 칸이라 요구하지 않는다.
    sub = re.compile(rf"^\s*소계\s+({NUM})\s+([\d.]+|-)(?:\s|$)")
    total = re.compile(rf"^\s*합계\s+({NUM})\s+([\d.]+|-)(?:\s|$)")
