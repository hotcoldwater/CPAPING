...
def requirements(original,file):
    ...
    if not re.search(r'이메일|이\s*메일|e-?mail',original,re.I):reasons.append('이메일 지원 방법을 확인해 주세요.')
    checks=[(r'지정\s*양식|자사\s*양식|당사\s*양식|첨부\s*(?:양식|파일)|\.hwp|첨부 양식:', '지정 양식이나 별도 첨부파일을 확인해 주세요.'),
            (r'자기소개서|자소서|경력기술서|증명서|증빙|성적표|동의서|추천서|포트폴리오|사진|자격증\s*(?:사본|스캔)', '추가 제출 서류가 있습니다. 업로드한 파일로 요건을 충족하는지 확인해 주세요.'),
            (r'(?:메일|이메일|e-?mail)\s*제목|제목\s*[:：]|파일\s*명|압축|참조|\bCC\b|온라인\s*(?:지원|접수)|홈페이지|채용\s*사이트|지원\s*링크', '메일 제목·파일명·지원 경로 등 별도 규칙을 확인해 주세요.'),
