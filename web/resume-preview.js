(()=>{
 ...
 function selectedFile(){const f=files.find(f=>f.id===$('resume-file-id').value);text('preview-file',f?.name||'이력서를 업로드해 주세요');if(!['uploading','error'].includes(window.cpResumeUploadState))text('resume-chosen',f?f.name+(f.id===savedFile?' · 새 파일을 올리지 않으면 이 파일을 유지합니다.':' · 업로드 완료'):'파일을 선택하면 바로 업로드됩니다.');}
