...
function form(){if(!$('resume-form'))return;files();const r=state.rule,f=r?.filters||{};formVersion=r?.updated_at||null;
 ...
 $('resume-mode').value=r?.mode||'review';$('resume-enabled').checked=r?.enabled===true;$('resume-consent').checked=false;$('resume-confirmed').checked=false;
...
try{
 ...
 $('resume-form').onsubmit=async e=>{e.preventDefault();if(busy||!ready||window.cpResumeSteps?.prepareSubmit()===false)return;const b=$('save-resume-rule');lock(true);b.textContent='저장 중…';let committed=false;try{const filters={employment:[$('resume-full').checked?'Full Time':null,$('resume-part').checked?'Part Time':null].filter(Boolean)};
 ...
 await api('resume-rule','PUT',{resume_file_id:$('resume-selected').value,applicant_name:$('applicant-name').value,subject:$('resume-subject').value,body:$('resume-body').value,filters,mode:$('resume-mode').value,enabled:$('resume-enabled').checked,updated_at:formVersion,file_confirmed:$('resume-confirmed').checked,auto_consent:$('resume-consent').checked});committed=true;await load();message(state.rule?.enabled?'저장했습니다. 지금 이후의 새 공고부터 선택한 방식으로 지원합니다.':'새 공고 지원이 꺼진 상태로 설정을 저장했습니다.');}catch(e){if(committed){ready=false;if($('retry-resume'))$('retry-resume').hidden=false;message('저장은 완료했지만 최신 설정을 확인하지 못했습니다. 다시 불러오기를 눌러 주세요.',true);}else message(e.message,true);}finally{lock(false);b.textContent='설정 저장하기 →';}};
