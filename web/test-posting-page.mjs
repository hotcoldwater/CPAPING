...
const render = (overrides = {}) => renderPostingPage({
  posting: {
    ...
    source: "kicpa:trainee", source_categories:["trainee"], recruitment_categories:["entry_cpa"], company_type:"회계법인", is_big4: true, employment_type: "Full Time",
...
test("빅4 수습 공고는 신입과 빅4를 함께 표시하고 원문으로 연결한다", () => {
  const html = render();
  assert.ok(html.includes("신입 CPA · 빅4"));
  assert.ok(html.includes('<span class="chip">회계법인</span>'));
  assert.ok(html.includes('href="https://example.com/posting"'));
  assert.ok(html.includes("수습CPA"));
});

test("빅4 경력 공고의 기존 표기를 유지한다", () => {
  const html = render({ source: "kicpa:cpa", recruitment_categories:["experienced_cpa"], company_type:"회계법인", career_min_years: 3, career_max_years: 5 });
  assert.ok(html.includes("경력 CPA · 빅4"));
