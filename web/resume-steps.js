...
(()=>{
 ...
 function summary(){
  const select=$('resume-file-id');const entries=[['이력서',$('current-file-name').textContent||'업로드 필요'],['지원자',$('applicant-name').value||'입력 필요'],['지원 조건',[$('resume-full').checked?'풀타임':'',$('resume-part').checked?'파트타임':''].filter(Boolean).join(' · ')||'선택 필요'],['발송 방식',$('resume-mode').value==='auto'?'자동발송':'수동발송'],['메일 제목',$('resume-subject').value],['일일 발송 한도','제한 없음']];
 ...
 function validate(index){
  if(index===1&&!$('resume-file-id').value)return error(index,$('resume-upload'),'이력서 파일을 업로드해 주세요.');
