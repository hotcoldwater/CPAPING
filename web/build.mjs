...
import { buildAssetFiles } from "./asset-builder.mjs";
...
for (const file of ["mail-connect.js", "career.css", "career.js", "career-template.json", "applications.js", "resume.js", "applications.css", "navigation.js", "navigation.css", "member.css", "resume-preview.js", "resume-design.css"]) copyFileSync(join(HERE, file), join(out, file));
...
// Content-addressed assets prevent old scripts from running against new page markup.
const versions=buildAssetFiles(out);
writeFileSync(join(HERE,'asset-versions.mjs'),'// Generated by web/build.mjs. Shared with live Pages Functions.\nexport default '+JSON.stringify(versions,null,2)+';\n');
// Revalidate HTML so a previous deployment cannot keep the old asset references alive.
writeFileSync(join(out,'_headers'),'/*\n  Cache-Control: no-cache, max-age=0, must-revalidate\n');
// 공개/회원 화면 모두 같은 배포의 스크립트를 사용한다.
function normalizeNavigation(dir){for(const entry of readdirSync(dir,{withFileTypes:true})){
 ...
 else if(entry.name.endsWith('.html')){const rel=file.slice(out.length).replace(/\\/g,'/').replace(/index\.html$/,'');writeFileSync(file,navigation(readFileSync(file,'utf8'),rel,versions),'utf8');}
}}
