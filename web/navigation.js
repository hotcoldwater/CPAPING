(() => {
 ...
 function careerAccess(allowed){document.querySelectorAll('[data-admin-href]').forEach(a=>{if(allowed){a.href=a.dataset.adminHref;a.removeAttribute('aria-disabled');a.removeAttribute('tabindex');}else{a.removeAttribute('href');a.setAttribute('aria-disabled','true');a.setAttribute('tabindex','-1');}});}
 ...
 async function update(){const seq=++sequence;let session=stored();if(!session){visible(false);careerAccess(false);shownUser=null;if(label)label.textContent='닉네임 설정';return;}visible(true);
  const owner=session.user?.id||session.access_token;if(shownUser!==owner){careerAccess(false);if(label)label.textContent='닉네임 확인 중…';}
  try{
   if(window.cpAuth){const {data}=await window.cpAuth.client.auth.getSession();session=data.session;if(!session){if(seq===sequence){visible(false);careerAccess(false);}return;}}
   ...
   careerAccess(p.career_admin===true);if(label)label.textContent=p.nickname||'닉네임 설정';shownUser=session.user?.id||session.access_token;retries=0;
  }catch{if(seq!==sequence)return;careerAccess(false);if(!shownUser&&label)label.textContent='닉네임 다시 확인';if(retries++<2)setTimeout(update,3000);}
 }
